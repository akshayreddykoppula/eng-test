...
public class CompositeUserServiceImpl implements CompositeUserService {
	...
	@Override
	public List<CreditCard> getUserCreditCardList(String userCreditCardUrl, Optional<String> creditCardState) throws Exception {
		...
		creditCardList = restTemplate.getForObject(userCreditCardUrl, CreditCardList.class);
			...
			if(ex.getMessage().contains(NOT_FOUND)) {
				throw new NotFoundException("The requested User Credit Card API URL: " + userCreditCardUrl + " could not be found.");
			} else {
				throw new Exception(ex.getMessage(), ex.getCause());
			}
		}
	...
	public List<Device> getUserDeviceList(String userDeviceUrl, Optional<String> deviceState) throws Exception {
		...
		try {
			...
			if(ex.getMessage().contains(NOT_FOUND)) {
				throw new NotFoundException("The requested User Device API URL: " + userDeviceUrl + " could not be found.");
			} else {
				throw new Exception(ex.getMessage(), ex.getCause());
			}
		}
